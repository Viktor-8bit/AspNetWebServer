...
public class ProcessController : ControllerBase
{
    ...
    public async Task<IActionResult> SendProcessAction([FromRoute] string hostname, [FromBody] List<JsonProcessAction> ActionProcesses)
    {
        ...
        MountedProcess last = _dbContext.MountedProcesses.Where(p => p.PcSender.hostname == hostname).OrderByDescending(x => x.MonutedIndex).FirstOrDefault<MountedProcess>();
        ...
        {
            _dbContext.ProcessActions.Add(new ProcessAction()
            {
                Name = aprocess.Name,
    ...
    public async Task<List<JsonProcessClient>> GetAllProcessByDate([FromRoute] string hostname, [FromRoute] DateTime date)
    {
        ...
        int mountedindex = 
            _dbContext.ProcessActions.Where(pa => pa.Date.Equals(date)).Select(p => p.MountedProcess).FirstOrDefault<MountedProcess>().MonutedIndex;
        
        // вытаскиваем начальное положение
        List<JsonProcessClient> start_processes = 
            _dbContext.MountedProcesses.Where(mp => mp.MonutedIndex == mountedindex).Select(mounted => new JsonProcessClient
            {
                Name = mounted.Name,
                Date = mounted.Date,
        ...
        List<ProcessAction> process_actions =
            _dbContext.ProcessActions.Where(pa => pa.MountedProcess.MonutedIndex == mountedindex && pa.Date <= date).OrderBy(pa => pa.Date).ToList<ProcessAction>();
        ...
        foreach (ProcessAction pa in  process_actions)
        {
            if (pa.Action.Equals(ProcessActions.Closed))
            {
                ...
                JsonProcessClient delete_process = 
                    start_processes.Where(pac => pac.ProcessId == pa.ProcessId && pac.Name == pa.Name).FirstOrDefault<JsonProcessClient>();
    ...
    // можем взять статичные процессы
    ...
    {
        return _dbContext.MountedProcesses.Where(p => p.PcSender.hostname == hostname && p.MonutedIndex == Mid).OrderByDescending(x => x.MonutedIndex).ToList<MountedProcess>();
    }
    ...
    {
           return _dbContext.MountedProcesses.Where(x => x.PcSender.hostname == hostname).GroupBy(x => x.Date).Select(x => x.Key).ToList();
    }
    
    [HttpGet("GetAllProcessActionDate/{hostname}")]
    public async Task<List<DateTime>> GetAllActionDate([FromRoute] string hostname)
    {
        return _dbContext.ProcessActions.Where(x => x.PcSender.hostname == hostname).GroupBy(x => x.Date).Select(x => x.Key).ToList();
    }
